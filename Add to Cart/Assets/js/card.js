    ...
    carts.map((val) => {
        ...
        tbl += `
            <tr>
                ...
                <td>₹ ${val.price}</td>   
                <td><input type="number" min="1" id="qty_${val.id}" onchange="editCart(${val.id})" value="${val.qty}" style=""></td>   
                <td>₹ ${val.price * val.qty}</td> 
                 <td><button onclick="deleteCart(${val.id})"><i class="fa-solid fa-trash-can delete" ></i></button></td>  
            </tr>`;
    });
